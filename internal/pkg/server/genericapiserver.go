...
import (
	...
	"wechatbot/internal/pkg/middleware"
	"wechatbot/pkg/log"
...
type GenericAPIServer struct {
	...
	*gin.Engine
	...
	enableSecureServer           bool
	insecureServer, secureServer *http.Server
}

func (s *GenericAPIServer) InitGenericAPIServer() {
	// do some setup
...
func (s *GenericAPIServer) InstallMiddlewares() {
	...
	s.Use(middleware.RateLimit(20, 10)) // 每秒 20个请求  最大刺突10个
	log.Infof("install middleware: %s", "ratelimit maxEventsPerSec:20 maxBurstSize:10")
...
func (s *GenericAPIServer) InstallAPIs() {
	// 开启健康检查 k8s从此接口判断是否发布成功
	if s.healthz && s.HealthzHandler != nil {
		// s.GET("/healthz", func(c *gin.Context) {
		// 	core.WriteResponse(c, nil, map[string]string{"status": "ok"})
		// })
		s.GET("/healthz", s.HealthzHandler)
	}
